<x-app-layout>
    <x-slot name="title">{{ __('app.zones.edit') }} - {{ config('app.name') }}</x-slot>

    @section('page-title', __('app.zones.edit'))

    <div class="max-w-2xl mx-auto">
        <div class="card">
            <h2 class="text-xl font-bold text-primary-700 mb-6">{{ __('app.zones.edit') }}: {{ $zone->name }}</h2>
                ...
                <div class="form-group">
                    ...
                    <select id="leader_id" name="leader_id" class="input @error('leader_id') input-error @enderror">
                        ...
                        @foreach($leaders as $leader)
                            <option value="{{ $leader->id }}" {{ old('leader_id', $zone->leader_id) == $leader->id ? 'selected' : '' }}>
                                {{ $leader->full_name }}
                            </option>
                        @endforeach
                    </select>

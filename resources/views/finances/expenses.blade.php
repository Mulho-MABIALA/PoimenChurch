<x-app-layout>
    <x-slot name="title">Dépenses - {{ config('app.name') }}</x-slot>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('finances.incomes') }}" class="text-gray-500 hover:text-primary-600 font-medium">
                Entrées
            </a>
    ...
    <div class="card mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            @foreach($expensesByCategory as $key => $expense)
                <div class="p-3 rounded-lg {{ $expense['total'] > 0 ? 'bg-red-50 border border-red-100' : 'bg-gray-50' }}">
                    <p class="text-xs text-gray-500 truncate" title="{{ $expense['label'] }}">{{ $expense['label'] }}</p>
                    <p class="font-semibold {{ $expense['total'] > 0 ? 'text-red-600' : 'text-gray-400' }}">
    ...
    <!-- Filters -->
    <div class="card mb-6">
        <form method="GET" action="{{ route('finances.expenses') }}" class="flex flex-wrap gap-4">
            ...
            <div class="w-48">
                <select name="expense_category" class="input">
                    <option value="">Toutes catégories</option>
                    @foreach($expenseCategories as $key => $label)
                        <option value="{{ $key }}" {{ request('expense_category') === $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            @if(request()->hasAny(['search', 'expense_category', 'year']))
                <a href="{{ route('finances.expenses') }}" class="btn-outline">Réinitialiser</a>
            @endif
        </form>
    </div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                {{ $transaction->expense_category_label }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            @if($transaction->funding_source === 'treasury')
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-700" title="Affecte le solde">
                                    Caisse
                                </span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-700" title="N'affecte pas le solde">
                                    Externe
                                </span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            ...
                            @if($transaction->vendor)
                                <span class="text-xs text-gray-400 block">{{ $transaction->vendor }}</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('finances.show', $transaction) }}" class="text-gray-400 hover:text-primary-600" title="Voir">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('finances.edit', $transaction) }}" class="text-gray-400 hover:text-yellow-600" title="Modifier">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

<x-app-layout>
    <x-slot name="title">{{ __('app.bacentas.add') }} - {{ config('app.name') }}</x-slot>

    @section('page-title', __('app.bacentas.add'))

    <div class="max-w-2xl mx-auto">
        <div class="card">
            <h2 class="text-xl font-bold text-primary-700 mb-6">{{ __('app.bacentas.add') }}</h2>

            <form method="POST" action="{{ route('bacentas.store') }}" class="space-y-4">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select id="shepherd_id" name="shepherd_id" class="input @error('shepherd_id') input-error @enderror">
                        ...
                        @foreach($shepherds as $shepherd)
                            <option value="{{ $shepherd->id }}" {{ old('shepherd_id') == $shepherd->id ? 'selected' : '' }}>
                                {{ $shepherd->full_name }}
                            </option>
                        @endforeach
                    </select>

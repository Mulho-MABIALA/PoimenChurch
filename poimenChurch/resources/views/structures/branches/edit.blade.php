<x-app-layout>
    <x-slot name="title">{{ __('app.branches.edit') }} - {{ config('app.name') }}</x-slot>

    @section('page-title', __('app.branches.edit'))
    ...
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('branches.update', $branch) }}" class="card">
            ...
            <h3 class="text-lg font-semibold text-primary-700 mb-4">{{ __('app.branches.edit') }}: {{ $branch->name }}</h3>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="form-group">
                        ...
                        <input type="text" id="address" name="address" value="{{ old('address', $branch->address) }}" class="input">
                    </div>

                    <div class="form-group">
                        <label for="city" class="label">{{ __('app.branches.city') }}</label>
                        <input type="text" id="city" name="city" value="{{ old('city', $branch->city) }}" class="input">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="form-group">
                        ...
                        <select id="leader_id" name="leader_id" class="input">
                            ...
                            @foreach($leaders as $leader)
                                <option value="{{ $leader->id }}" {{ old('leader_id', $branch->leader_id) == $leader->id ? 'selected' : '' }}>
                                    {{ $leader->full_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select id="assistant_leader_id" name="assistant_leader_id" class="input">
                            <option value="">--</option>
                            @foreach($leaders as $leader)
                                <option value="{{ $leader->id }}" {{ old('assistant_leader_id', $branch->assistant_leader_id) == $leader->id ? 'selected' : '' }}>
                                    {{ $leader->full_name }}
                                </option>
                            @endforeach
                        </select>

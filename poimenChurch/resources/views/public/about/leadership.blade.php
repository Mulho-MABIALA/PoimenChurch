<x-public-layout>
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="grid lg:grid-cols-2">
                        <div class="bg-gradient-to-br from-primary-700 to-primary-900 p-8 lg:p-12 flex items-center justify-center">
                            @if($seniorPastor->photo)
                                <img src="{{ $seniorPastor->photo_url }}" alt="{{ $seniorPastor->name }}"
                                     class="w-64 h-64 rounded-full object-cover ring-4 ring-white/20">
                            @else
                        ...
                        <div class="p-8 lg:p-12">
                            ...
                            <h2 class="text-3xl font-bold text-primary-800 mb-2">{{ $seniorPastor->name }}</h2>
                            <p class="text-gray-500 mb-6">{{ $seniorPastor->title }}</p>
                            @if($seniorPastor->bio)
                                <p class="text-gray-600 leading-relaxed mb-6">{{ $seniorPastor->bio }}</p>
                            @endif
                            @if($seniorPastor->twitter_url || $seniorPastor->facebook_url)
                            <div class="flex gap-4">
                                @if($seniorPastor->twitter_url)
                                <a href="{{ $seniorPastor->twitter_url }}" target="_blank" rel="noopener noreferrer"
                                   class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center text-primary-700 hover:bg-primary-200 transition-colors">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"/>
                                    </svg>
                                </a>
                                @endif
                                @if($seniorPastor->facebook_url)
                                <a href="{{ $seniorPastor->facebook_url }}" target="_blank" rel="noopener noreferrer"
                                   class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center text-primary-700 hover:bg-primary-200 transition-colors">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="container mx-auto px-4">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                ...
                <div class="bg-background rounded-2xl p-6 text-center hover:shadow-lg transition-shadow">
                    <div class="w-32 h-32 mx-auto mb-4 flex items-center justify-center">
                        @if($member->photo)
                            <img src="{{ $member->photo_url }}" alt="{{ $member->name }}"
                                 class="w-32 h-32 rounded-full object-cover ring-2 ring-primary-100">
                        @else
                    ...
                    <h3 class="text-xl font-bold text-primary-800 mb-1">{{ $member->name }}</h3>
                    <p class="text-gold-600 font-medium mb-3">{{ $member->title }}</p>
                    @if($member->bio)
                        <p class="text-gray-600 text-sm">{{ $member->bio }}</p>
                    @endif
                    @if($member->twitter_url || $member->facebook_url)
                    <div class="flex justify-center gap-3 mt-4">
                        @if($member->twitter_url)
                        <a href="{{ $member->twitter_url }}" target="_blank" rel="noopener noreferrer"
                           class="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center text-primary-700 hover:bg-primary-200 transition-colors">
    ...
    @if(!$seniorPastor && $teamMembers->isEmpty())
    <!-- Empty state -->
    <section class="py-24 bg-background">
        <div class="container mx-auto px-4 text-center">
            ...
            <h2 class="text-2xl font-bold text-primary-800 mb-3">Informations bientot disponibles</h2>
            <p class="text-gray-600">Les informations sur notre equipe de leadership seront bientot publiees.</p>
        </div>
    </section>
    @endif
